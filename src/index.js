...
// Extracted from falladviser-us/index.html · 98097 bytes of source logic
...
(async function(){await openDB();await loadAll();await updateTierBadge();await loadEstateContext();if(!state.active||!TABS.find(t=>t.id===state.active))state.active='dashboard';render();})();
  try {
    ...
    const manifestRes = await fetch('https://sjgant80-hub.github.io/fall-autopilot-kit/manifests/falladviser-us.json');
